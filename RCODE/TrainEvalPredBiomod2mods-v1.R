...
flTrain <- list.files(paste(projectDir,"/DATA_/TABLES/_TRAIN_DATASETS/",sep=""), pattern="_TrainData_", 
                      full.names = TRUE)
flTrain <- flTrain[grepl(".rds$", flTrain)]

# List matrices used for train with info on Pseuso-absence sets
flMat <- list.files(paste(projectDir,"/DATA_/TABLES/_TRAIN_DATASETS/",sep=""), pattern="_TrainMatrix_", 
                    full.names = TRUE)
flMat <- flMat[grepl(".rds$", flMat)]

# List files containing selected variables from previous steps
flSelVars <- list.files(paste(projectDir,"/DATA_/TABLES/_TRAIN_DATASETS/",sep=""), pattern="_SelectedVars_", 
                        full.names = TRUE)
