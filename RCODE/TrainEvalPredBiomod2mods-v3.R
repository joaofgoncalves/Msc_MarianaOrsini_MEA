...
lens <- c(length(flTrain), length(flMat), length(flSelVars))


if(!((lens[1]==lens[2]) & (lens[1]==lens[3]) & (lens[2]==lens[3]))){
  stop("Different lengths in objects with train data, train matrix and selected vars sources!!")
}
...
for(i in 1:length(flTrain)){
  ...
  spNameInit  <- trimws(trainData[1,"SpeciesName"], which = "both")
  ...
  #predVars <- predVars[!(predVars %in% clNames)]
  ...
  # for each technique
  selMods <- twoStepBestModelSelection(biomodModelOut = myBiomodModelOut, 
                                       evalMetric = "TSS", 
                                       nrBestAlgos = 6, 
